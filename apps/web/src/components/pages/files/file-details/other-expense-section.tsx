...
export default function OtherExpenseSection() {
  const { year, fileNo, data, totals } = useFileDetailsContext();
  const expenseData = data.other;
  const total = totals.other;

  return (
    <Card className="flex flex-col divide-y-2 px-2! text-sm">
      <div className="flex items-center justify-between gap-2 pb-1">
        <div className="font-semibold text-base">Other Expense</div>
        <DialogFileExpense
          fileNo={fileNo}
          year={year}
          data={data}
